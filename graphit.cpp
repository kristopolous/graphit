...
int8_t Graphit::interpolate(vector<float>&interpolated, vector<float> data, int32_t width) 
{
...
int8_t Graphit::rasterize(vector<float> &rasterized, vector<float> interpolated, int32_t height, float min, float max) 
{
  size_t m_charlen = Graphit::m_charset.size() - 1;
  ...
  {
    rasterized.push_back( (int32_t)(m_charlen * height * ((*it - min) / delta)));
  }

  return 0;
}

int8_t Graphit::plot(vector<wstring> &buffer, vector<float> rasterized, int32_t width, int32_t height) 
{
  size_t m_charlen = m_charset.size() - 1;

  int32_t row_ix, col_ix, tmp, current_floor, current_ceiling;
...
int8_t Graphit::process(vector<wstring> &output, vector<float> data, int32_t width, int32_t height, float min, float max) 
{
  int8_t ret = 0;
...
int8_t Graphit::print_curses(vector<float> data, WINDOW*win, float min, float max) 
{
  int8_t ret = 0;
  ...
  ret = this->process( output, data, width, height, min, max);
  ...
  int32_t row = 0;
  for(vector<wstring>::const_iterator it = output.begin(); it != output.end(); it++) 
  {
    mvwaddnwstr(win, row++, 0, (const wchar_t *)((*it).c_str()), (*it).size());
  }
...
int8_t Graphit::print_direct(vector<float> data, int32_t width, int32_t height, float min, float max) 
{
  int8_t ret = 0;
